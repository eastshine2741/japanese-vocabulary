...
배포는 k3s 클러스터로만 운영 (로컬 docker-compose 사용 안 함). 백엔드/DB는 `./deploy.sh`로 띄우고, 프론트엔드는 로컬에서 실행:
...
Batch 모듈에는 ingress가 없음 (api만 외부 노출). 로컬에서 batch endpoint를 호출하려면 `kubectl port-forward` 사용:

```bash
NS=$(git rev-parse --abbrev-ref HEAD)         # 또는 namespace 직접 지정
kubectl port-forward -n "$NS" deployment/batch 8081:8081
# 다른 셸에서:
curl -X POST 'http://localhost:8081/api/dev/freeze-consume?runDate=2026-04-26'
```
