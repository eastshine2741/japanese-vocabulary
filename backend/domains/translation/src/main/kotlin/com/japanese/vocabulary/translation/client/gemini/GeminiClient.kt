...
class GeminiClient(
    ...
    companion object {
        ...
        private val SEGMENTATION_PROMPT = """
            ...
            ## 핵심 원칙: 사전 lookup 가능한 최소 어휘 단위로 분절하라
            이 단계의 출력은 Jisho 사전 조회에 그대로 사용된다. 하나의 word는 원칙적으로
            Jisho에서 찾을 수 있는 하나의 표제어(dictionary headword)에 대응해야 한다.
            문맥상 자연스러운 구/절/관용 해석 단위를 하나의 word로 묶지 마라.

            ## 묶으면 안 되는 것
            - 명사/대명사/부사 + 조사: 何の為 → 何 / の / 為, どこかで → どこか / で, ここまで → ここ / まで, 今の → 今 / の.
            - 형용사·부사의 연용형 + 동사: 上手くいって → 上手く / いって. 上手くいく처럼 묶지 마라.
            - 의문사/부사 + 동사 + 종조사: どうしようか → どう / しよう / か.
            - 동사가 명사를 수식하는 구: 行く宛 → 行く / 宛.
            - 따옴표, 괄호, 공백, 쉼표, 숫자, 라틴 문자만으로 된 기호. surface에 기호를 섞지 말고,
              내부에 일본어 어휘가 있으면 그 어휘만 출력하라. 일본어 어휘가 없으면 words=[].

            ## 그대로 한 word로 둘 수 있는 것
            - Jisho 표제어로 등재된 고정 부사·접속사·복합어: ちゃんと, きっと, ずっと, どうして,
              なんだか, とにかく, だから, 成れの果て, 飛び立つ, 巡り会う, 弾き出す.
            - 단, 끝의 は·を·が·の·に·で·まで·も·か·って·た·ない·ている 같은 조사/조동사가
              별도 문법 성분이면 반드시 분리한다.

            ## 분절 규칙
            - surface: 원문에 나타난 그대로의 표면형(활용형 포함). 원문 순서대로 빠짐없이. 표면형들을 순서대로 이으면 (공백/기호 제외) 원문을 덮어야 한다.
            - dictionaryForm: 그 word 하나를 Jisho에서 조회할 사전 표제형. 조사/조동사를 붙인 구 형태를 dictionaryForm으로 만들지 마라.
