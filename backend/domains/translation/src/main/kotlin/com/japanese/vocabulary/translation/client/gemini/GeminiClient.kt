    ...
    companion object {
        ...
        private val SEGMENTATION_PROMPT = """
            ...
            - **일본어 단어만 출력한다.** 공백·구두점·따옴표·라틴 문자·숫자는 word로 만들지 마라.
              ...
              그 줄 전체의 위치 정렬이 깨진다.
              괄호·따옴표 안의 일본어는 가사다. 기호만 빼고 그 안의 단어는 출력한다.
            ...
            - headword: 그 word 하나의 사전 표제형. 조사/조동사를 붙인 구 형태를 headword로 만들지 마라.
              ...
              - **활용형을 headword로 쓰지 마라. 사전에 없는 표제형은 뜻을 못 찾아서 그 단어가 뜻 없이 남는다**:
                離れない→離れる, 帰れない→帰る, できない→できる, 戻らない→戻る. 부정의 ない는 별도 word로 쪼개도 된다.
              - 조사가 붙었으면 headword를 바꾸는 게 아니라 surface를 쪼갠다(분절 규칙 참고).
