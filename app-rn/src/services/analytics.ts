 ...
 * 체류 시간은 앱에서 재지 않고, BigQuery 에서 song_detail_open 이후
 * 다음 screen_view 까지의 event_timestamp 차이로 계산한다.
 */
type AnalyticsEvent =
  ...
  | { name: 'song_detail_open'; params: { song_id: number; origin: string } };
...
export function trackSongDetailOpen(songId: number, origin: string): void {
  track({ name: 'song_detail_open', params: { song_id: songId, origin } });
}
