...
채널: `development`(Android dev), `production`(정식). 절차는 `../docs/runbooks/mobile-ota-release.md`.
...
```bash
BUILD_ENV=prod EAS_UPDATE_CHANNEL=production NATIVE_RUNTIME_VERSION=1.2.1 EXPO_PUBLIC_OTA_UPDATE_NUMBER=1 \
...
- `eas update` 는 `eas.json` build profile 의 `env` 를 자동으로 쓰지 않는다. prod OTA 는 `BUILD_ENV=prod EAS_UPDATE_CHANNEL=production EXPO_PUBLIC_OTA_UPDATE_NUMBER=N` 을 명시한다.
