...
Lyric providers run in order and each hit carries a `LyricMatchConfidence`. `STRONG` (the provider matched the artist — LrcLib `/api/get`, an artist-name hit in LrcLib search, any VocaDB match) ends the search. `WEAK` (LrcLib duration-only match, no artist evidence) is held while the remaining providers run and used only when none of them returns a `STRONG` hit; a same-titled song by another artist is the likelier reading of a weak hit, which is how Sohbana『恋』once received Conton Candy's lyrics.
