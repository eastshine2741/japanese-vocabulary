...
- 서버에 저장되는 가사 자체의 문제는, UX 조정으로 해결 가능한가?
  - 불가능. 한번에 가사 한줄만 번역결과 보여준다던가, 스와이프로 바꾼다던가 해도 달라질건없다. 어차피 '가사를 사용한다' 이므로
- 이 앱이 공정이용 조항에 해당하는가?
...
- 가사 자체 문제의 근본적 해결: 라이센스 계약이 필요함
  - jasrac, komca(한국음악저작권협회)
    - 내 앱은 사용자가 아무 곡이나 선택할 수 있으므로, 개별 곡마다 계약하는게 아니라 일괄 계약하는 방법이 가능한지 찾아봐야함
    - 근데 또 jasrac, komca에서 관리안되는 곡이 있을수있다. 그런 곡은 따로 다뤄야함. 개별 연락을 하든...
  - musixmatch, lyricfind 등으로 가능한지는, 그런 회사들과 저작자 사이의 권리관계를 확인해봐야할듯함
- 개인으로보는단체나 법인 필요한가?
  - 영리적으로 한다면 있는 게 좋아보인다. 보통 업종 선택하고 그에 맞게 영업활동한다고 함. 세금처리 등 문제도 있는듯
- AI 대신 사람이 직접 번역하고 분석하면 괜찮나?
  - 그럼 2차적저작물 문제가 생긴다... 2차적저작권리는 권리자의 배타적 권리이므로 허락을 받아야한다.
  - 2차적저작물 권리는 음자협에서 관리안해서 개별 허락이 필요할수도... 음자협에서 관리하는건 배포권 복제권 정도다. 있는것 그대로 사용하는것만 관리한다. 변형은 그 변형의 정도를 저작권자가 허락해야하므로 음자협 관할이 아니다.
- 가사를 아예 버리면 괜찮나?
  - 당연히 가사문제는 사라지긴하는데, 플래시카드의 예문으로 가사를 사용한다면 여전히 문제됨
- 권리자침해신고 등 기능이 공정이용으로 간주되는데에 얼마나 효과가 있는가?
  - 공정이용 참작은 될수있는데
  - 있어봤자, 권리자가 나한테 매출 뱉어라 등 손해배상 요구를 여전히 할 수 있다
- 수익의 규모는 저작권 문제에 얼마나 영향이 있는가?
  - 수익에 따라서 손해배상금은 달라지지만
  - 침해는 침해임. 소액이더라도 손해배상금 발생할수있고 법원 판단에 달려있음
- 비슷한 앱은 왜 괜찮은가? 왜 괜찮은지를 내가 알수있는 방법이 있나?
  - 가지는 문제 자체는 나와 같음
  - 보통은 권리자가 모르거나 알고도 냅둔다. 근데 규모가 커지면 문제제기 될 수 있다.
  - 걔네가 어떻게 계약했는지는 검색해서 알 수 없다. 고지하는 형태가 아니므로.
  - 알려면 직접 컨택해야할듯.
- 곡 공유 기능 추가해도 문제없나?
  - 곡 공유 자체는 문제가 없는데(단순 url 공유이므로)
