...
class SettingsViewModel(private val repository: FlashcardRepository = FlashcardRepository()) {
    ...
    fun loadSettings() {
        ...
        scope.launch {
            try {
                ...
                state.value = SettingsState.Loaded(requestRetention = settings.requestRetention)
            } catch (e: Exception) {
    ...
    fun saveSettings() {
        ...
        scope.launch {
            try {
                val result = repository.updateSettings(UserSettingsDTO(requestRetention = current.requestRetention))
